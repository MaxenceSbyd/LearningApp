import type { MCWordExercise } from '../../types';
...
export default function MCWord({ ex, picked, onPick }: Props) {
  return (
    <div>
      ...
      <div className="bg-sand border border-taupe rounded-sm p-8 text-center mb-8">
        <div className="font-display text-5xl text-charcoal mb-2">{ex.term}</div>
        {ex.reading && (
          <div className="text-warm-grey text-sm tracking-widest">{ex.reading}</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {ex.options.map((opt) => {
          ...
          const isCorrect = opt === ex.correctTranslation;
